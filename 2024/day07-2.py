...
def check_equation(acc, total, values):
    ...
    if acc > total:
        ...
        return (
            check_equation(mul(acc, values[0]), total, values[1:])
            or check_equation(add(acc, values[0]), total, values[1:])
            or check_equation(concat(acc, values[0]), total, values[1:])
        )


def main():
    repo_root = os.popen("git rev-parse --show-toplevel").read().strip()
    file_path = os.path.join(repo_root, "2024/input/input07-2")
