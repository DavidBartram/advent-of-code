...
def move_file(files,gaps,pos):
    file_positions = list(files.keys())
    file_positions.sort(reverse=True)

    gap_positions = list(gaps.keys())
    gap_positions.sort()
    ...
    while len(gap_positions) > 0:
        ...
        if gaps[gap_pos]["size"] >= file["size"] and gap_pos < pos:
            ...
            if gaps[gap_pos]["size"] > file["size"]:
                ...
                gap_positions.pop(0)
                gap_positions = [new_gap_pos] + gap_positions
...
def move_files(files,gaps):
    ...
    positions = list(files.keys())
    positions.sort(reverse=True)
    while len(positions) > 0:
        if len(positions) % 1000 == 0:
            print(len(positions))
        files, gaps = move_file(files,gaps,positions[0])
        positions.pop(0)
...
def main():
    ...
    files,gaps = move_files(files, gaps)
